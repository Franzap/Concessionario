...
public class Esercitazione1 {
	...
	private static void comandoCancellaVeicolo(Concessionario c, Scanner scan) {
		...
		try {
			...
			System.out.println();
			scan.nextLine();
			Esercitazione1.comandoCancellaVeicolo(c, scan);
		...
		if(Esercitazione1.ripetereComando(scan,"Sicuro di voler eliminare il veicolo "+ numeroVeicolo +"? Scrivi 1 per \"SI\" E 2 per \" no\" " )) {
	...
	private static void comandoModificaVeicolo(Concessionario c, Scanner scan) {
		...
		try {
			...
			System.out.println();
			scan.nextLine();
			Esercitazione1.comandoModificaVeicolo(c, scan);
			...
			if(Esercitazione1.ripetereComando(scan,"Vuoi ripetere l'operazione? 1: Ripetere 0: Uscire")) {
				Esercitazione1.comandoModificaVeicolo(c, scan);
			}
	...
	private static void comandoAggiungiVeicolo(Concessionario c, Scanner scan) {
		...
		try {
			scelta=Esercitazione1.ripetereComando(scan, "Inserisci 0 per aggiungere un Automobile o inserisci 1 per aggiungere una Macchina Agricola");
			scan.nextLine();
			...
			scan.nextLine();
			Esercitazione1.comandoAggiungiVeicolo(c,scan);
		}	
	...
	private static boolean ripetereComando(Scanner scan,String msg) {
		...
		try {
			...
			if(valore==1) {
				...
				scan.nextLine();
				Esercitazione1.ripetereComando(scan,msg);
			}
		} catch (InputMismatchException e) {
			System.out.println("Hai sbagliato input, RIPROVA!!");
			scan.nextLine();
